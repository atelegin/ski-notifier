#!/usr/bin/env python3
"""Promote high-quality extended XC candidates to resorts.yaml."""
...
REGISTRY_PATH = ROOT / "ski_notifier" / "xc_registry.yaml"

TARGET_ADDITIONS = 20
MIN_DISTANCE_KM_FROM_EXISTING = 2.0
MIN_DISTANCE_KM_BETWEEN_NEW = 3.0

UA = {"User-Agent": "ski-notifier-promote-xc/1.0"}
BERGFEX_XC_SOURCES = [
    ("DE", "https://www.bergfex.de/baden-wuerttemberg/langlaufen/"),
    ("DE", "https://www.bergfex.de/deutschland/langlaufen/"),
    ("CH", "https://www.bergfex.ch/schweiz/langlaufen/"),
    ("AT", "https://www.bergfex.at/oesterreich/langlaufen/"),
]
...
def is_hard_blocked_name(name: str) -> bool:
    """Names that should never be promoted as standalone XC resorts."""
    s = name.strip().lower()
    if "verbindungsloipe" in s:
        return True
    if re.fullmatch(r"loipe\\s*\\d+", s):
        return True
    if re.fullmatch(r"loipe\\s*[a-z]", s):
        return True
    return False
...
def norm_name(name: str) -> str:
    s = name.lower()
    s = s.replace("loipe", " ").replace("langlauf", " ").replace("nordic", " ")
    s = re.sub(r"\b(rot|blau|grün|gruen|schwarz|leicht|schwer|plus|km|runde)\b", " ", s)
    s = re.sub(r"\d+", " ", s)
    return " ".join(s.split())
...
def quality_name(name: str) -> bool:
    ...
    if re.fullmatch(r"loipe\s*[a-z]", s):
        return False
    # Keep names that still look like place-based XC objects.
    return "loipe" in s or "langlauf" in s or "nordic" in s
...
def norm_match(s: str) -> str:
    ...
    for t in ["langlauf", "loipe", "nordic", "center", "zentrum", "nordic center"]:
        s = s.replace(t, " ")
    s = re.sub(r"[^a-z0-9]+", " ", s)
    return " ".join(s.split())


def fetch_bergfex_xc_allowlist() -> List[dict]:
    """Fetch XC place titles from Bergfex listings (DE/CH/AT)."""
    rows = []
    seen = set()
    pattern = re.compile(
        r'href="(/[^"]*/langlaufen/[^"/]+/)"[^>]*title="([^"]+)"',
        re.IGNORECASE,
    )
    for country, src in BERGFEX_XC_SOURCES:
        try:
...
def is_bergfex_match(
    candidate_name: str, allowlist: List[dict], country_hint: str | None = None
) -> Tuple[bool, str | None]:
    """Return match flag + URL when candidate matches Bergfex allowlist."""
    ...
    for row in allowlist:
        if country_hint and row.get("country") != country_hint:
            continue
        bn = row["name_norm"]
        if not bn:
            continue
...
def make_entry(c: dict, country: str, id_set: set[str], website_override: str | None = None) -> dict:
    base = f"{country.lower()}_{slugify(c['name'])}_xc"
    rid = base
    n = 2
    while rid in id_set:
        rid = f"{base}_{n}"
    ...
    return {
        "id": rid,
        "name": f"{c['name']} ({country}) — XC",
        "country": country,
        "type": "xc",
        "drive_time_min_from_konstanz": int(c["approx_drive_time_min"]),
        "drive_time_note": "Auto-promoted from OSM XC registry (needs field validation)",
        "website": website_override
        or f"https://www.openstreetmap.org/?mlat={lat:.6f}&mlon={lon:.6f}#map=13/{lat:.6f}/{lon:.6f}",
        "notes": "Auto-promoted from extended OSM nordic candidate. Verify trail status/entry point manually.",
        "access": {
        ...
        "points": {
            "low": {
                "label": "Trailhead / parking (OSM)",
                "lat": lat,
                "lon": lon,
            },
            "high": {
                "label": "Nearby upper loop sector (approx)",
                "lat": round(lat + 0.01, 6),
                "lon": round(lon + 0.01, 6),
            },
        },
        "costs": {
...
def main() -> None:
    ...
    bergfex_allowlist = fetch_bergfex_xc_allowlist()
    ...
    for c in registry.get("extended", []):
        ...
        if is_hard_blocked_name(c["name"]):
            continue
        is_bf, bf_url = is_bergfex_match(c["name"], bergfex_allowlist)
        if not is_bf and not quality_name(c["name"]):
            continue
        row = dict(c)
        row["_bergfex_bw_match"] = is_bf
        row["_bergfex_url"] = bf_url
        candidates.append(row)

    # Name-level dedupe by normalized key.
    by_key: Dict[str, dict] = {}
    for c in sorted(
        candidates,
        key=lambda x: (0 if x.get("_bergfex_bw_match") else 1, x["approx_drive_time_min"], x["name"]),
    ):
    ...
    for c in sorted(
        deduped,
        key=lambda x: (0 if x.get("_bergfex_bw_match") else 1, x["approx_drive_time_min"], x["distance_km"], x["name"]),
    ):
        lat = float(c["lat"])
        lon = float(c["lon"])
        pt = (lat, lon)

        if any(haversine_km(pt, p) < MIN_DISTANCE_KM_FROM_EXISTING for p in existing_xc_points):
            continue
